...
int insertHash( HashTable H, HashEntry newitem ) {
    ...
    if ( H[probe].used == 0 ) { /* if the entry is not taken yet */
        ...
        fprintf( logfile, "Inserted entry with key %s into HT!\n",
            newitem.key );
        if ( slotIndex < HASHSIZE ) slotIndex++; //update the HT utilization
        ...
        if ( HCD < HASHTHLD || slotIndex < HASHSIZE ) HCD++; /* record the collision */
        else {
            printHashDistribution( H );
            clearTable( H );
        }
        return -1;
    } else {
...
int retrieveHash( HashTable H, HashEntry target ) {
    ...
    while ( H[probe].used != 0 && strcmp( target.key, H[probe].key )
            && pc <= HASHSIZE / 2 ) {
        pc++;
        probe = ( probe + increment ) % HASHSIZE;
        increment += 2;
...
int constructHash( xmlNode * a_node, HashEntry Item, HashTable ht ) {
    ...
    if ( xmlStrlen( id_buf ) > 0 && xmlStrlen( seq_num_buf ) > 0
            && xmlStrlen( timestamp_buf ) > 0
            && xmlStrlen( datetime_buf ) > 0 ) {
        /*
         ran = new_rand();
...
int get_id( xmlNode * a_node, xmlChar * buffer ) {
    ...
    for ( i = 0, j = 0, cur_node = a_node; cur_node; i++, j++, cur_node =
            cur_node->next ) {
            ...
            if ( f == 1 ) {
                if ( cur_node->properties != NULL ) {
                    if ( cur_node->properties->type == XML_ATTRIBUTE_NODE ) {
                        ...
                        if ( id_buf == NULL ) continue;

                        if ( id_buf != NULL ) {
                            id = atoi( (char *)id_buf );
                            fprintf( logfile, "get_id: id is: %d\n", id );
                            fprintf( logfile, "get_id: id is: %s\n", id_buf );
                            xmlStrPrintf( buffer, xmlStrlen( id_buf ) + 1,
                                (char*)id_buf );
                        }
                        f = 0;
                        xmlFree( id_buf );
            ...
            if ( f == 1 ) {
                if ( cur_node->properties != NULL ) {
                    if ( cur_node->properties->type == XML_ATTRIBUTE_NODE ) {
                        ...
                        //just testing....
                        if ( seq_num_buf == NULL )
                            seq_num_buf = xmlGetProp( cur_node,
                                (xmlChar *)"seq_num" );

                        if ( seq_num_buf == NULL ) continue;
                        ...
                        if ( seq_num_buf != NULL ) {
                            ...
                            fprintf( logfile, "get_seq_num: seq_num is: %s\n",
                                seq_num_buf );
                            xmlStrPrintf( buffer, xmlStrlen( seq_num_buf ) + 1,
                                (char*)seq_num_buf );
                        }
                        f = 0;
                        xmlFree( seq_num_buf );
            ...
            if ( f == 1 ) {
                if ( cur_node->properties != NULL ) {
                    if ( cur_node->properties->type == XML_ATTRIBUTE_NODE ) {
                        ...
                        //just testing....
                        if ( timestamp_buf == NULL )
                            timestamp_buf = xmlGetProp( cur_node,
                                (xmlChar *)"timestamp" );

                        if ( timestamp_buf == NULL ) continue;

                        if ( timestamp_buf != NULL ) {
                            timestamp = atoi( (char *)timestamp_buf );
                            fprintf( logfile,
                                "get_timestamp: timestamp is: %d\n",
                                timestamp );
                            fprintf( logfile,
                                "get_timestamp: timestamp is: %s\n",
                                timestamp_buf );
                            xmlStrPrintf( buffer,
                                xmlStrlen( timestamp_buf ) + 1,
                                (char*)timestamp_buf );
                        }
                        f = 0;
                ...
                if ( cur_node->properties != NULL ) {
                    if ( cur_node->properties->type == XML_ATTRIBUTE_NODE ) {
                        ...
                        if ( datetime_buf == NULL )
                            datetime_buf = xmlGetProp( cur_node,
                                (xmlChar *)"datetime" );

                        if ( datetime_buf == NULL ) continue;
                        ...
                        if ( datetime_buf != NULL ) {
                            ...
                            fprintf( logfile, "get_datetime: datetime is: %s\n",
                                datetime_buf );
                            xmlStrPrintf( buffer, xmlStrlen( datetime_buf ) + 1,
                                (char*)datetime_buf );
                        }
                        f = 0;
